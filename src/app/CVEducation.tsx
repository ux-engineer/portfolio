...
export default function CVEducation() {
  return (
    <section id="CVEducation" className="grid grid-cols-10 gap-4 my-10">
      ...
      <div className="col-span-9 lg:col-span-9 sm:pr-10 pb-5">
        {education.content.map(entry => (
          <div key={entry.title} className="grid grid-cols-6 gap-2 text-sm md:text-base">
            ...
            { entry.body ?
              <p className="block col-span-6 text-sm">{entry.body}</p>
              : ''
            }
          </div>
        ))}
